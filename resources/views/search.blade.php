...
<div class="mb-4">
    ...
    <p class="text-muted">Found {{ $videos->total() }} results</p>
</div>
                ...
                <div class="card h-100 border-0" style="background: transparent;">
                    <a href="{{ route('videos.show', $video) }}" class="text-decoration-none">
                        <div class="position-relative" style="padding-top: 150%; overflow: hidden; border-radius: 4px;">
                            ...
                            @if($video->type === 'tv')
                                <span class="position-absolute top-0 end-0 m-2 badge bg-danger">TV</span>
                            @else
                                <span class="position-absolute top-0 end-0 m-2 badge bg-primary">Movie</span>
                            @endif
                        </div>
                    </a>
                    <div class="card-body p-2">
                        ...
                        @if($video->category)
                            <small class="text-muted">{{ $video->category->name }}</small>
                        @endif
                    </div>
                </div>
            </div>
